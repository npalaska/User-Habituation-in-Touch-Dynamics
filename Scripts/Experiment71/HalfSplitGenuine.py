...
"""Break the genuine file into 2 sub-files"""
...
while user <= 31:
    ...
    path = parentdirectory + '\Output Files\Experiment71\Genuine User\GenuineUser' + str(
        user)
    ...
    lengthoffile = (len(arr_genuine) - len(
        arr_genuine) % 2) / 2  # set the length of sub-file
    while fileno <= 2:
        x = arr_genuine[(
                            fileno - 1) * lengthoffile: fileno * lengthoffile]  # get the sub-files from complete file
        np.savetxt(
            "../../Output Files/Experiment71/Genuine User/GenuineUser" + str(
