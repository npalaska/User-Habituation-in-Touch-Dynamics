...
# Create the Folder 'ROC Characteristics Decision Tree' to save the roc characteristic file
...
path = parentdirectory + '\Output Files\Experiment70\Decision Tree\ROC Characteristics Decision Tree'
...
while user <= 31:
    while block <= 3:
        while fold <= 10:
            ...
            clf_scaled = DecisionTreeClassifier()  # Classifier with data scaled
            clf_scaled.fit(sample_train_scaled, target_train)
            clf = DecisionTreeClassifier()  # Classifier with actual data (without scaling)
            clf.fit(sample_train, target_train)

            # Predict the target values with the classifier.
            ...
            np.savetxt(
                "../../Output Files/Experiment70/Decision Tree/ROC Characteristics Decision Tree/1-" + str(
...
np.savetxt(
    "../../Output Files/Experiment70/Decision Tree/DecisionTreeScaled_EER.csv",
    eervalues, delimiter=",")

aucvalues = np.transpose(arr_auc_scaled.reshape(93, 10))
np.savetxt(
    "../../Output Files/Experiment70/Decision Tree/DecisionTreeScaled_Auc.csv",
    aucvalues, delimiter=",")
aucvalues = np.transpose(arr_auc.reshape(93, 10))
np.savetxt(
    "../../Output Files/Experiment70/Decision Tree/DecisionTree_Auc.csv",
    aucvalues, delimiter=",")
...
np.savetxt(
    "../../Output Files/Experiment70/Decision Tree/DecisionTreeScaled_MeanEER.csv",
    arr_eer.reshape(31, 3), delimiter=",")
np.savetxt(
    "../../Output Files/Experiment70/Decision Tree/DecisionTreeScaled_MeanAuc.csv",
    arr_auc_scaled.reshape(31, 3), delimiter=",")
np.savetxt(
    "../../Output Files/Experiment70/Decision Tree/DecisionTree_MeanAuc.csv",
