...
path = parentdirectory + '\Output Files\Test set Group Size 10 for evaluation\slide5/7-11'
...
arr_eer = arange(1085, dtype=float32).reshape(1085, 1)
arr_auc = arange(1085, dtype=float32).reshape(1085, 1)
        ...
        while fold <= 5:
            ...
            traindata1 = genfromtxt(
                '../../Output Files/Experiment100/12splits/GIUser/User' + str(
                    user) + '/1-' + str(user) + '-1-9-' + str(fold) + '.csv', dtype=float,
                delimiter=',')

            traindata2 = genfromtxt(
                '../../Output Files/Experiment100/12splits/GIUser/User' + str(
                    user) + '/1-' + str(user) + '-1-10-' + str(fold+1) + '.csv', dtype=float,
                delimiter=',')

            traindata3 = genfromtxt(
                '../../Output Files/Experiment100/12splits/GIUser/User' + str(
                    user) + '/1-' + str(user) + '-1-7-' + str(fold+2) + '.csv', dtype=float,
                delimiter=',')

            traindata4 = genfromtxt(
                '../../Output Files/Experiment100/12splits/GIUser/User' + str(
                    user) + '/1-' + str(user) + '-1-8-' + str(fold+3) + '.csv', dtype=float,
                delimiter=',')

            traindata5 = genfromtxt(
                '../../Output Files/Experiment100/12splits/GIUser/User' + str(
                    user) + '/1-' + str(user) + '-1-11-' + str(fold+4) + '.csv', dtype=float,
                delimiter=',')


            traindata = np.vstack((traindata1, traindata2, traindata3, traindata4, traindata5))

            testdata = genfromtxt(
                '../../Output Files/Experiment100/12splits/GIUser/User' + str(
                    user) + '/1-' + str(user) + '-1-' + str(
                    block) + '-' + str(fold+5) + '.csv', dtype=float,
                delimiter=',')
            ...
            while rowno < len(target_test_prediction):
                G = 0
                I = 0
                while counter <= min(10, LastCounter):
                    ...
                    rowcount -= 1
                if G > I:
                    target_test_prediction[rowno-10:rowno] = np.ones(10)
                else:
                    target_test_prediction[rowno-10:rowno] = np.zeros(10)
                counter = 1
...
eervalues = np.transpose(arr_eer.reshape(217, 5))
np.savetxt(
    "../../Output Files/Test set Group Size 10 for evaluation/slide5/7-11/RandomForest_EER.csv",
    eervalues, delimiter=",")

aucvalues = np.transpose(arr_auc.reshape(217, 5))
np.savetxt(
    "../../Output Files/Test set Group Size 10 for evaluation/slide5/7-11/RandomForest_Auc.csv",
    aucvalues, delimiter=",")
...
np.savetxt(
    "../../Output Files/Test set Group Size 10 for evaluation/slide5/7-11/RandomForest_MeanEER.csv",
    arr_eer.reshape(31, 7), delimiter=",")

np.savetxt(
    "../../Output Files/Test set Group Size 10 for evaluation/slide5/7-11/RandomForest_MeanAuc.csv",
